...
function ToggleGameState({gameState, setGameState, challengeGame, setChallenge}) {

  const [buttonText, setButtonText] = useState("Start a new game!");
 
  function updateGameState(challengeSetting, challengeGame) {
    ...
    //SET BUTTON TO "END GAME" IF THE LOAD CHALLENGE BUTTON IS SELECTED
      ...
      if(challengeGame === CHALLENGES_STATE.CHALLENGE_1){
        setChallenge(CHALLENGES_STATE.CHALLENGE_1);
      }
    else if(challengeGame === CHALLENGES_STATE.CHALLENGE_2){
        setChallenge(CHALLENGES_STATE.CHALLENGE_2);
      }
    else if(challengeGame === CHALLENGES_STATE.CHALLENGE_3){
        setChallenge(CHALLENGES_STATE.CHALLENGE_3);
      }
    ...
    else if ( !(challengeSetting) && (gameState === GAME_STATE.CHALLENGE_MODE) ) {
